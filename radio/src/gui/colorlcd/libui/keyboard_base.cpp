...
#if defined(SIMU)
static bool forceKeyboardObjectCreateFailureForTest = false;

void keyboardForceObjectCreateFailureForTest(bool force)
{
  forceKeyboardObjectCreateFailureForTest = force;
}
#endif
...
{
#if defined(SIMU)
  if (forceKeyboardObjectCreateFailureForTest) return nullptr;
#endif
...
Keyboard::Keyboard(coord_t height, bool fullScreen) :
    NavWindow(MainWindow::instance(),
              {0, fullScreen ? 0 : LCD_H - height, LCD_W, height}),
...
bool Keyboard::setField(const FieldBinding& binding)
{
  ...
  auto rollback = [&]() {
    if (activeKeyboard == this) activeKeyboard = nullptr;
    clearField(true);
    return false;
  };
