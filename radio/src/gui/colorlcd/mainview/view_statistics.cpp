 ...
 public:
      ...
      Window(parent, rect)
  {
    ...
    if (prefix != nullptr) {
      ...
      new (std::nothrow) StaticText(this, {0, 1, prefixSize, rect.h},
                                    prefix, COLOR_THEME_PRIMARY1_INDEX,
                                    FONT(XS));
    }
    new (std::nothrow) DynamicNumber<uint32_t>(
        this, {prefixSize, 0, rect.w - prefixSize, rect.h}, numberHandler);
  }
};
...
void DebugViewPage::build(Window* window)
{
  ...
  new (std::nothrow) StaticText(line, rect_t{}, STR_FREE_MEM_LABEL);
  new (std::nothrow) DynamicNumber<int32_t>(
  ...
  if (heartbeatCapture.valid)
    ...
    new (std::nothrow) StaticText(window, grid.getFieldSlot(), "---");
#endif
  ...
  if (serialGetModePort(UART_MODE_GPS) >= 0) {
    ...
    new (std::nothrow) DynamicText(
        line, rect_t{0, 0, DBG_B_WIDTH, DBG_B_HEIGHT},
        [] {
          const char* fixText = gpsData.fix ? STR_GPS_FIX_YES : STR_GPS_FIX_NO;
          return std::string(fixText ? fixText : "");
        });
    new (std::nothrow) DebugInfoNumber<uint8_t>(
        line, rect_t{0, 0, DBG_B_WIDTH, DBG_B_HEIGHT},
