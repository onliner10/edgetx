...
class GuardedMLinkFrame
{
 public:
  explicit GuardedMLinkFrame(size_t size):
      mappingSize(0),
      mapping(nullptr),
      frame(nullptr)
  {
...
TEST(MLink, shortMultiRx9PacketDoesNotReadPastFrame)
{
  GuardedMLinkFrame frame(7);
  ASSERT_TRUE(frame.isValid());

  frame.data()[2] = 0x13;

  processMLinkPacket(frame.data(), true, 7);
}
