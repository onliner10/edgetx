...
TEST(ColorBitmapBuffer, BuiltinIconAllocationFailureLeavesDrawableMask)
{
  const pid_t pid = fork();
  ASSERT_GE(pid, 0);
...
TEST(ColorBitmapBuffer, CanvasCreateFailureKeepsData)
{
  const pid_t pid = fork();
  ASSERT_GE(pid, 0);
  ...
  if (pid == 0) {
    alarm(2);
    _exit(bitmapBufferCanvasCreateFailureKeepsDataForTest() ? 0 : 1);
  }
...
TEST(ColorBitmapBuffer, ResizeAllocationFailurePreventsLvglOverread)
{
  const pid_t pid = fork();
  ASSERT_GE(pid, 0);
  ...
  if (pid == 0) {
    alarm(2);
    _exit(bitmapBufferResizeAllocationFailurePreventsLvglOverreadForTest() ? 0
                                                                          : 1);
  }
...
TEST(ColorBitmapBuffer, LZ4BitmapAllocationFailureInvalidatesBuffer)
{
  const pid_t pid = fork();
  ASSERT_GE(pid, 0);
  ...
  if (pid == 0) {
    alarm(2);
    _exit(lz4BitmapBufferAllocationFailureInvalidatesBufferForTest() ? 0 : 1);
  }
