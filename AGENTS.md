...
**Always create a fresh git worktree before making any changes.** Never work directly in the main checkout.

```sh
SUFFIX=$(date +%Y%m%d-%H%M%S)
mkdir -p ~/git/Edge16.worktrees
git worktree add ~/git/Edge16.worktrees/task-${SUFFIX} main
cd ~/git/Edge16.worktrees/task-${SUFFIX}
```

Then restart opencode so the new cwd is your worktree. The simulator MCP server resolves the git worktree root from its cwd via `git rev-parse --show-toplevel`, so builds, screenshots, and fixtures are all scoped to your worktree under `build/ui-harness/`. Multiple agents working in parallel each get their own independent `build/` tree — no collisions.

When done, clean up:

```sh
cd ~/git/Edge16  # leave the worktree
...
- The UI harness writes simulator builds under `build/ui-harness` by default (relative to the worktree root, thanks to `git rev-parse --show-toplevel`). Set `EDGETX_UI_BUILD_ROOT=/tmp/edgetx-ui-build` only when you intentionally want build output outside the worktree.
